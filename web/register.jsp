...
<script type="text/javascript">
    ...
    function check_phone(phone) {
        ...
        if (reg.test(phone)) {
            $.ajax({
                url: "${root}/servlet/user?methodName=checkPhone",
                data: {"phone": phone},
                dataType: "json",
                type: "post",
                success: function (res) {
                    console.log(res)
                    if ("exist" == res) {
                        $("#phone_notice").html("手机号已经被注册").css("color", "red")
                    } else {
                        $("#phone_notice").html("手机号可用").css("color", "green")
                    }
                }
            })
        } else {
            $("#phone_notice").html("手机号码不符合规范").css("color", "red")
        }
    }
</script>
